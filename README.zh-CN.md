...
可安装到 Codex 和 Claude Code 的 Skill，以及一个无需登录的中文网页工具。它们会抓住：处境变成了对你的判决的那一刻。
...
Codex + Claude Code · 可安装到手机桌面的中文工具 · English + 中文 · 不追踪、不记录 · 不用治疗师口吻 · 不强行积极

## 使用中文网页工具

**[打开“纠正自我负面 Inner Talk”工具](https://slowrecover.github.io/inner-voice/app/zh/)**

这是一个手机优先的 PWA。它先识别人格判决、总是／从不、灾难化和读心四类语言变化，再请用户自己写下可观察的事实。工具不会替用户生成“正确答案”。

- 不登录、不注册，不调用 API，没有数据库或统计工具。
- 输入的文字只在当前页面中处理，刷新或关闭后即消失。
- 离线功能只缓存网页文件，绝不缓存用户输入。
- iPhone：用 Safari 打开，选择“分享 → 添加到主屏幕”。
- Android：用 Chrome 打开，选择“安装应用”或“添加到主屏幕”。

网页源码位于 [`app/zh`](app/zh)。以后如需英文网页，可增加 `app/en`；四个 Skill 仍独立放在 `skills/` 下。
